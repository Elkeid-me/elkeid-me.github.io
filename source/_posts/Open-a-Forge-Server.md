...
> 警告：长期以来，笔者只游玩原版 MC，对模组不甚熟悉。因此，本文内容较为简略，且可能有诸多错误。
...
![](Open-a-Forge-Server/folder_1.jpg)
...
![](Open-a-Forge-Server/output.jpg)
...
现在，运行 `run.bat`。命令行窗口会弹出提示，要求你修改 `eula.txt`。根据文章 {% post_link Open-an-MC-Server %} 修改即可。

关闭命令行窗口，再次运行 `run.bat` 即可。

![](Open-a-Forge-Server/output2.jpg)

对于 `server.properties` 的配置，建议阅读 {% post_link Open-an-MC-Server %}。安装 mod 时，放入 `mods` 文件夹即可。
