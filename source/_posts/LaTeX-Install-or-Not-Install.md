...
LaTeX 就是个排版工具。如果你用 Word 之类的足够熟练，那其实没有必要学 LaTeX。最近 Typst 信徒和 TeXmacs（以及国产 Fork 墨干编辑器）信徒传教很疯狂，对于后者我要多说点。[私货：锐评墨干编辑器](#私货：锐评墨干编辑器)
...
[TeX Live](https://tug.org/texlive/) 由 TeX 用户组推出，每年更新，如今是 TeX Live 2023。支持 Windows、Linux 和 Unix，在 macOS 上有 MacTeX。其问题是包管理器的图形界面实在是太不友好了（悲），而且每年封装完镜像之后，新发布的包只能等待第二年再封装。例如：2022 年发布的 [fixdif](https://ctan.org/pkg/fixdif) 包只能等 2023 年的 TeX Live 镜像，或者使用命令 `tlmgr install fixdif`
手动安装。但如果使用 MiKTeX，你只需在文档中使用 `\usepackage{fixdif}`， 在编译时，会自动安装缺少的包。
...
![](LaTeX-Install-or-Not-Install/miktex-console.png)_MiKTeX Console_

此时，“检索源”是 `<互联网上的随机存储库>`。强烈建议点击右侧的“更改”，切换到一个国内的源，例如清华源或者科大源。然后，点击“检查更新”：

![](LaTeX-Install-or-Not-Install/update.png)_MiKTeX Update_
...
首先，请读到这里的各位先读一读墨干编辑器开发者的言论[^3]。平心而论，TeXmacs 是好软件，而墨干也做了很多性能优化和本土化的尝试。**但是现在说“TeXmacs 功能上早已取代 LaTeX”为时尚早**。我在昨天（2023/8/15）尝试了最新版的墨干，我就想问，**一个连 OpenType 特性都不支持，动不动就闪退崩溃，文档语言设定为中文时英文 Ligature 莫名失效的软件，是怎么好意思说“功能上早已取代 LaTeX”的**？现如今，Word 都支持基本的 OpenType 特性，TeX 这边呢，LuaHBTeX 引入了 Harfbuzz，搞出这种活是轻而易举的：

![](LaTeX-Install-or-Not-Install/luatex-loads-vf.png)_Source Serif 4 Variable 字体，LuaHBLaTeX 编译_

而墨干连 Ligature 都会莫名失效；官方给的解决方案真的很 ad-hoc。可以看[我在 gitee 提的 issue](https://gitee.com/XmacsLabs/mogan/issues/I7TGZS)。

至于 OpenType 特性，昨天[我也提了 issue](https://gitee.com/XmacsLabs/mogan/issues/I7TH3U)，官方说一年之内解决。拭目以待。

[^1]: [MathML Support - Adobe InDesign](https://indesign.uservoice.com/forums/601021-adobe-indesign-feature-requests/suggestions/32805088-mathml-support)
[^2]: [如何看待 Typst？](https://www.zhihu.com/question/591143170)
[^3]: [如何看待 TeXmacs？未来能完全取代 LaTeX 吗？ - 沈浪熊猫儿的回答 - 知乎](https://www.zhihu.com/question/487769817/answer/2792055709)
