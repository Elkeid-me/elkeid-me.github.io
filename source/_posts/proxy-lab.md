...
所以，`std::shared_mutex` 摆烂！下面称 `std::shared_mutex::shared_lock()` 为“读锁定”，`std::shared_mutex::lock()` 为“写锁定”。

好，下一个问题。缓存里面存什么？显然 Key 是 URL，value 是指向缓存对象的指针、时间戳以及对象的大小。那缓存就直接设计为 `std::unordered_map<std::string, cache_block>`。`cache_block` 的大致结构如下：
...
只要仍存在对 o 的引用，那就不要从内存中释放 o。这不就是垃圾回收吗？但是 C++ 又有什么垃圾回收呢？~~`std::declare_reachable`（无端）~~答案是 `std::shared_ptr`，可以视为引用计数 GC。
...
- 由于奇怪的操作系统调度，线程 A 被挂起；
- 线程 B、C、D……向缓存写入数据，写的对象太多以至于空间不够了，对象 o 被从缓存驱逐。缓存不再持有 o，o 的引用计数为 1，此时 o 的内存还没有被释放。
- 线程 A 又被调度。
- 线程 A 将 o 写入 tcp 套接字。
...
多个进程同时查找 o，会同时更新时间戳。但我认为这无伤大雅。

所以 `cache_block` 的结构就这么愉快的决定了：
...
public:
    ...
    fd_wrapper(fd_wrapper &&other) : m_fd{other.m_fd}
    {
        other.m_fd = -1;
    }
