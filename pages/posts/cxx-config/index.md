...
前往 `mingw-builds-binaries` 的 GitHub Release 页面，选择文件名中包含 `x86_64`、`posix`、`seh` 和 `ucrt` 的版本。截至本文发布，应该下载 `x86_64-14.1.0-release-posix-seh-ucrt-rt_v12-rev0.7z`。
...
终于到了最复杂的部分——clangd 本体的配置。对于初学者（单文件编译），请看 [`.clangd`](#clangd-2)；如果你正在使用某种构建系统，请看 [`compile_commands.json`](#compile-commands-json)。
...
GNU Make 无法自动生成 `compile_commands.json`。但是有一个网站 [Generate Compilation Database from GNU make output online](https://texttoolkit.com/compilation-database-generator)。具体来说，你需要：
