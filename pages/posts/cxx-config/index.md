...
1. 编辑器：用来编写和查看纯文本文件的软件，例如Windows自带的记事本。本文使用[Visual Studio Code](https://code.visualstudio.com/)（简称VS Code）。
::: tip
注意：写字板、Word不属于纯文本编辑器，而应属于富文本编辑器，不在我们的讨论范围内。
:::
2. 编译器：将源代码翻译为可执行的二进制文件。本文使用GCC。广义上GCC是C、C++、Objective-C、Fortran、Ada、Go和D等多种语言的编译器（GNU Compiler Collection），狭义上GCC仅指C语言编译器（GNU C Compiler）。而本文所指的GCC介于两者之间：即C语言编译器和C++ 编译器。
3. 调试器：单步执行、打断点等功能，调试程序。本文使用GDB。
...
前去[VS Code官网](https://code.visualstudio.com/)下载并安装，只需要跟着安装程序的提示走就可以了。
...
点击文件资源管理器的地址栏，将这一路径**复制**。接下来，打开Windows搜索（快捷键 <kbd>Win</kbd> + <kbd>S</kbd>），输入“环境变量”，点击“编辑系统环境变量”：
...
环境变量设置是否正确该如何检查呢？<kbd>Win</kbd> + <kbd>R</kbd> 打开“运行”，输入cmd并回车，打开命令行界面：
...
此外，也可以直接安装[完整的LLVM](https://github.com/llvm/llvm-project/releases)，它包含了clangd。
...
按 <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>P</kbd> 打开VSCode的命令面板，输入“用户设置”，选择“首选项: 打开用户设置(JSON)”。
...
你需要用VS Code打开一个**文件夹**作为你的**工作区**。如果你是初学者，习惯于单文件编译/运行，那么我推荐你找一个恰当的位置新建文件夹。如果你正在进行一些项目，那项目文件夹就是你的工作区。
...
- [生成compile_commands](https://xmake.io/#/zh-cn/plugin/builtin_plugins?id=%e7%94%9f%e6%88%90compile_commands)
